            ...
            <div class="grid grid-cols-1 xl:grid-cols-5 gap-6">
                ...
                <div class="xl:col-span-3 space-y-6">
                    ...
                    <div class="cyber-card p-4">
                        <h3 class="text-sm font-semibold text-white mb-4 flex items-center gap-2">
                            ...
                            Uploaded Error Files (<?php echo $attachments->num_rows; ?>)
                        </h3>
                            ...
                            <?php while ($att = $attachments->fetch_assoc()): ?>
                                <a href="<?php echo $att['filepath']; ?>" target="_blank" class="flex items-center gap-3 p-3 bg-[#0f0f15] rounded-lg hover:bg-[#1a1a2e] transition-colors">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-xs text-[#666]">from <?php echo htmlspecialchars($att['uploaded_by_name'] ?? 'Unknown user'); ?> &middot; <?php echo round($att['filesize']/1024, 1); ?> KB</p>
                                    </div>
                                </a>
                            <?php endwhile; ?>
                        </div>
                ...
                <div class="xl:col-span-2 space-y-6">
                    ...
                    <div class="cyber-card p-5">
                        <h3 class="text-sm font-semibold text-white mb-4 flex items-center gap-2">
                            <i data-lucide="activity" class="w-4 h-4 text-[#00ff88]"></i>
                            Status &amp; Actions
                        </h3>
                        
                        <!-- Current Status -->
                        <div class="mb-4">
                            <p class="text-[10px] text-[#666] uppercase tracking-wider mb-2">Current Status</p>
                            <div class="flex items-center gap-3 p-3 rounded-lg bg-[#0f0f15] border border-[#1a1a2e]">
                                <span class="w-2.5 h-2.5 rounded-full shrink-0" style="background-color: <?php
                                    $dot_colors = ['open' => '#f59e0b', 'in_progress' => '#3b82f6', 'resolved' => '#00ff88', 'closed' => '#6b7280'];
                                    echo $dot_colors[$ticket['status']] ?? '#00ff88';
                                ?>;"></span>
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-semibold text-white capitalize"><?php echo str_replace('_', ' ', $ticket['status']); ?></p>
                                    <p class="text-[10px] text-[#666] mt-0.5">Updated <?php echo date('M d, Y H:i', strtotime($ticket['updated_at'])); ?></p>
                                </div>
                            </div>
                        </div>
                        ...
                        <form method="POST">
                            ...
                            <select name="new_status" class="cyber-select mb-3">
                                <option value="open" <?php echo $ticket['status'] == 'open' ? 'selected' : ''; ?>>Open - Not yet worked on</option>
                                <option value="in_progress" <?php echo $ticket['status'] == 'in_progress' ? 'selected' : ''; ?>>In Progress - Being worked on</option>
                                <option value="resolved" <?php echo $ticket['status'] == 'resolved' ? 'selected' : ''; ?>>Resolved - Problem is fixed</option>
                                <option value="closed" <?php echo $ticket['status'] == 'closed' ? 'selected' : ''; ?>>Closed - No more action needed</option>
                            </select>
                            <?php if ($ticket['status'] != 'resolved'): ?>
                            <label class="block text-[10px] text-[#666] uppercase tracking-wider mb-2">Resolution Notes</label>
                            <textarea name="solution" class="cyber-textarea mb-3" placeholder="Describe how the problem was fixed..."></textarea>
                            <?php endif; ?>
                    ...
                    <div class="cyber-card p-5">
                        <h3 class="text-sm font-semibold text-white mb-4 flex items-center gap-2">
                            <i data-lucide="user-check" class="w-4 h-4 text-[#00ff88]"></i>
                            Assign Technician
                        </h3>
                        <form method="POST">
                    ...
                    <div class="cyber-card p-5">
                        <h3 class="text-sm font-semibold text-white mb-4 flex items-center gap-2">
                            <i data-lucide="info" class="w-4 h-4 text-[#00ff88]"></i>
                            Details
                        </h3>
                        <div class="space-y-3 text-xs">
                            <div class="flex justify-between items-start gap-3">
                                <span class="text-[#666]">Created By</span>
                                <span class="text-[#ccc] text-right"><?php echo htmlspecialchars($ticket['created_by_name']); ?></span>
                            </div>
                            <div class="flex justify-between items-start gap-3">
                                <span class="text-[#666]">Assigned To</span>
                                <span class="text-[#ccc] text-right"><?php echo $ticket['assigned_to_name'] ? htmlspecialchars($ticket['assigned_to_name']) : '<span class="text-[#444]">Unassigned</span>'; ?></span>
                            </div>
                            <div class="flex justify-between items-start gap-3">
                                <span class="text-[#666]">Department</span>
                                <span class="text-[#ccc] text-right"><?php echo $ticket['department']; ?></span>
                            </div>
                            <div class="flex justify-between items-start gap-3">
                                <span class="text-[#666]">Category</span>
                                <span class="text-[#ccc] text-right capitalize"><?php echo $ticket['category']; ?></span>
                            </div>
                            <div class="flex justify-between items-start gap-3">
                                <span class="text-[#666]">Priority</span>
                                <span class="text-[#ccc] text-right capitalize"><?php echo $ticket['priority']; ?></span>
                            </div>
                        </div>
                    </div>
