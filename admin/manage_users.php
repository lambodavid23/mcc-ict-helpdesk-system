...
$tech_count = $conn->query("SELECT COUNT(*) as total FROM users WHERE role = 'technician'")->fetch_assoc()['total'];
            ...
            <div class="cyber-card p-4 mb-6">
                ...
                <form method="POST" class="flex flex-wrap gap-4 items-end">
                    ...
                    <div class="flex-1 min-w-[140px]">
                        <label class="block text-[10px] text-[#666] uppercase tracking-wider mb-2">Role</label>
                            ...
                            <?php if ($users && $users->num_rows > 0): ?>
                                <?php while ($user = $users->fetch_assoc()): ?>
                                    <tr data-search="<?php echo strtolower($user['name'] . ' ' . $user['email'] . ' ' . $user['department']); ?>">
                                        ...
                                        <td>
                                            <?php 
                                            $role_class = 'badge-' . ($user['role'] == 'admin' ? 'admin' : ($user['role'] == 'technician' ? 'tech' : 'user'));
                                            ...
                                            <?php if ($user['id'] != $_SESSION['user_id']): ?>
                                                <div class="flex items-center gap-2">
                                                    <form method="POST" class="flex items-center gap-2">
                                                        ...
                                                        <select name="new_role" class="cyber-select w-28 text-xs">
                                                            <option value="user" <?php echo $user['role'] == 'user' ? 'selected' : ''; ?>>User</option>
                                                            <option value="technician" <?php echo $user['role'] == 'technician' ? 'selected' : ''; ?>>Technician</option>
                                                            <option value="admin" <?php echo $user['role'] == 'admin' ? 'selected' : ''; ?>>Admin</option>
                                                        </select>
                                                        <button type="submit" class="cyber-btn-secondary px-2 py-1 text-xs">Update</button>
                                                    </form>
