        ...
        switch ($_POST['action']) {
            case 'add_technician':
                ...
                if (empty($name) || empty($specialization) || empty($email) || empty($password)) {
                    ...
                    $check_email = "SELECT id FROM technicians WHERE email = '$email_escaped' LIMIT 1
                                    UNION ALL SELECT id FROM admins WHERE email = '$email_escaped' LIMIT 1
                                    UNION ALL SELECT id FROM users WHERE email = '$email_escaped' LIMIT 1";
                    ...
                    if ($check_result && $check_result->num_rows > 0) {
                        ...
                        $insert_query = "INSERT INTO technicians (name, specialization, phone, email, password) 
                                       VALUES ('$name', '$specialization', '$phone', '$email', '$hashed_password')";
            ...
            <div class="cyber-card p-4 mb-6">
                ...
                <form method="POST" class="flex flex-wrap gap-4 items-end">
                    ...
                    <div class="flex-1 min-w-[200px]">
                        ...
                        <input type="email" name="email" class="cyber-input" placeholder="tech@mcc.co.zw" required>
                    </div>
                    <div class="flex-1 min-w-[150px]">
                        <label class="block text-[10px] text-[#666] uppercase tracking-wider mb-2">Password</label>
                        <input type="password" name="password" class="cyber-input" placeholder="Login password" required>
                    </div>
